...
- (BOOL)isEqual:(id)object {
	...
	if (self.reportRowId != otherParameter.reportRowId)
		return NO;
	if (![self.uuid isEqual:otherParameter.uuid])
		return NO;
...
- (NSUInteger)hash
{
	NSString *hashString = [NSString stringWithFormat:@"%ld:%ld:%ld:%@", (long)_rowId, (long)_type, (long)_reportRowId, _uuid];
...
{
	return [NSString stringWithFormat:@"ReportParameter RowId:%ld, type:%@, reportRowId:%ld, uuid:%@", (long)_rowId, NSStringFromReportParameterType(_type), (long)_reportRowId, _uuid];
}
...
- (nonnull id)copyWithZone:(nullable NSZone *)zone {
	YYGReportParameter *newParameter = [[YYGReportParameter alloc] initWithRowId:_rowId type:_type reportRowId:_reportRowId uuid:_uuid];
	return newParameter;
}
