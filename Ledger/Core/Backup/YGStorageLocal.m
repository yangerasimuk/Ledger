...
- (NSArray <YGBackup *>*)backups {
    ...
    NSString *path = [NSString stringWithFormat:@"%@/%@", [YGTools documentsDirectoryPath], @"Backup"];
    
    // Check for Backup dir existance
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error = nil;
    if(![fileManager fileExistsAtPath:path]){
        if(![fileManager createDirectoryAtPath:path withIntermediateDirectories:NO attributes:nil error:&error]){
            if(error){
                NSLog(@"Fail to create local backup directory. Error: %@", [error description]);
            }
        }
    }
...
- (NSString *)copyWorkDbToBackup {
    ...
    // new full path
    NSString *newFullName = [NSString stringWithFormat:@"%@/Backup/%@", [YGTools documentsDirectoryPath], newName];
...
- (void)copyToWorkDbBackup:(YGBackup *)backup {
    ...
    // backup full path
    NSString *backupFullName = [NSString stringWithFormat:@"%@/Backup/%@", [YGTools documentsDirectoryPath], backup.name];
...
- (void)removeOldBackups:(NSArray <YGBackup *>*)backups {
    ...
    NSString *backupDirectory = [NSString stringWithFormat:@"%@/%@", [YGTools documentsDirectoryPath], @"/Backup/"];
    ...
    for (YGBackup *backup in backups){
        NSString *oldBackupFullName = [backupDirectory stringByAppendingPathComponent:backup.name];
