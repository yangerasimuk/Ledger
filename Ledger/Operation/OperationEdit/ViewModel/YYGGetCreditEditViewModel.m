...
@implementation YYGGetCreditEditViewModel

- (NSString *)title {
    return NSLocalizedString(@"OPERATION_EDIT_FORM_GET_CREDIT_TITLE", @"Title for get credit operation");
}
...
- (NSString *)titleForHeaderInSection:(NSInteger)section {
    ...
    switch(section) {
        case 0:
            break;
        case 1:
            title = NSLocalizedString(@"OPERATION_EDIT_FORM_GET_CREDIT_SECTION", @"Section title for get credit in operation edit form");
            break;
        case 2:
        case 3:
            break;
...
- (id)selectViewModelForRow:(NSInteger)row inSection:(NSInteger)section supposedCurrency:(YGCategory *)supposedCurrency {
    if(row == 0 && section == 1) // source
        return [YYGEntitySelectViewModel viewModelWith:YGEntityTypeDebt customer:YYGEntitySelectForOperationEditSource counterpartyType:YYGCounterpartyTypeCreditor allowCurrencies:self.allowDebtCurrencies supposedCurrency:supposedCurrency];
    else if(row == 2 && section == 1) // target
