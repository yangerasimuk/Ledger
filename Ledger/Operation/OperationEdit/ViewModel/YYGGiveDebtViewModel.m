...
@implementation YYGGiveDebtViewModel

- (NSString *)title {
    return NSLocalizedString(@"OPERATION_EDIT_FORM_GIVE_DEBT_TITLE", @"Title for give debt operation");
}
...
- (NSString *)titleForHeaderInSection:(NSInteger)section {
    ...
    switch(section) {
        case 0:
            break;
        case 1:
            title = NSLocalizedString(@"OPERATION_EDIT_FORM_GIVE_DEBT_SECTION", @"Section title for give debt in operation edit form");
            break;
        case 2:
        case 3:
            break;
...
- (id<YYGRowIdAndNameIdentifiable>)defaultSource {
    return [super defaultEntityWith:YGEntityTypeAccount counterpartyType:YYGCounterpartyTypeNone];
}
...
- (BOOL)isOnlyChoiceSource {
    return [super isOnlyChoiceEntityWith:YGEntityTypeAccount counterpartyType:YYGCounterpartyTypeNone];
}
        ...
        case 3:                 // Actions
            switch (row) {
                ...
                case 1:         // Save & add
                    ...
                    break;
                default:
                    @throw [NSException exceptionWithName:@"YYGGiveDebtEditViewModel heightForRow:inSection: fails." reason:@"Unknown row number." userInfo:nil];
            }
            break;
        default:
            @throw [NSException exceptionWithName:@"YYGGiveDebtEditViewModel heightForRow:inSection: fails." reason:@"Unknown section number." userInfo:nil];
    }
...
- (id)selectViewModelForRow:(NSInteger)row inSection:(NSInteger)section supposedCurrency:(YGCategory *)supposedCurrency {
    if(row == 0 && section == 1) // source
        ...
        return [YYGEntitySelectViewModel viewModelWith:YGEntityTypeDebt customer:YYGEntitySelectForOperationEditTarget counterpartyType:YYGCounterpartyTypeDebtor allowCurrencies:self.allowDebtCurrencies supposedCurrency:supposedCurrency];
    else
        return nil;
}

- (id)selectViewModelForRow:(NSInteger)row inSection:(NSInteger)section {
    @throw [NSException exceptionWithName:@"YYGGiveDebtEditViewModel selectViewModelForRow:inSection: fails." reason:@"Method can not be called." userInfo:nil];
}
